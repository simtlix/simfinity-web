...
export const FormItems = ({fields, parentFieldName, form, openForResult, entity, span=12}) =>{
    ...
    const notEmbeddedFields = fields.filter(item => !(item.extensions?.relation?.embedded))
    const embeddedFields = fields.filter(item => (item.extensions?.relation?.embedded))
    ...
    return <>
        ...
        <Space style={{width:"100%"}} direction="vertical">
            ...
            <Col span={24}><Card style={{width:"100%"}}>
            {renderFormFields}
        </Card></Col>
